...
if exist sam_front.c cl %CF% sam.c sam_lex.c sam_morph.c sam_pos.c sam_norm.c sam_front.c sam_say.c /Fe:..\build\%ARCH%\sam_say.exe || exit /b 1
rem standalone build with the voice data compiled in (run tools\embed_data.py first)
if exist sam_data.c cl %CF% /DSAM_EMBEDDED sam.c sam_lex.c sam_morph.c sam_pos.c sam_norm.c sam_front.c sam_say.c sam_data.c /Fe:..\build\%ARCH%\sam_standalone.exe || exit /b 1
goto :eof
